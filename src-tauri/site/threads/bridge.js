// Twister's bridge into threads.com. Injected at document start into every
// Threads tab, main frame only, on every page load, after common.js.
//
// Threads is Meta's web stack: generated class names, no test ids, and a
// navigation of icons named by their SVG's aria-label. Twister changes
// nothing about the page's layout here — only the font and the smoothness
// of scrolling — and watches the rest. Rust substitutes the three
// __TWISTER_*__ placeholders.
//
...
(() => {
  ...
  function installStyles() {
    common.installSheet('threads/niceties.css', CSS)
  ...
  // one whose glyph is labelled Profile, since nothing else names it.
  ...
  function findProfile() {
    for (const glyph of document.querySelectorAll('svg[aria-label="Profile"]')) {
      ...
      if (!match || match[1] === reportedHandle) return
  ...
  // ── Going places ──────────────────────────────────────────────────────────
  // A link the page drew for the path is the in-app transition its router
  // understands; failing that, a pushState and a popstate, which Meta's
  // router takes the way it takes Back.
  function go(path) {
